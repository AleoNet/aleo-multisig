...
2. Signing by authorized signers until the threshold is met, done by calling the `sign` transition (for Aleo signers) or `sign_ecdsa` transition (for ECDSA signers) on the `multisig_core.aleo` program, passing it the `wallet_id` and `signing_op_id`. ECDSA signatures also require a unique `nonce` parameter to prevent signature replay attacks - the signature is computed over the hash of `{wallet_id, signing_op_id, nonce}`. Signatures must be submitted before the operation expires (i.e., current block height < start height + block expiration).
